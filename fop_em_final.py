...
if __name__ == '__main__':
    ...
    # 4. Setup inversion with constraints
    inv = pg.Inversion(verbose=True)
    inv.setForwardOperator(fop)
    
    # Use log transform to keep values positive and set bounds
    trans = pg.trans.TransLog(lowerBound=1e-3, upperBound=10.0)
    inv.setTransModel(trans)
    
    # Set better inversion parameters
    inv.setLambda(1.0)  # Lower regularization
    inv.setMaxIter(15)
    ...
    try:
        ...
        # 6. Plot results with FIXED dimensions
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 6))
        
        # Plot 1: Resistivity model - FIXED plotting
        n_layers = len(resistivity_model)  # Should be 3
        
        # Create proper depth arrays for plotting
        if len(depths) == 2:  # depths = [1, 5]
            # Layer structure: 0-1m, 1-5m, 5m-inf
            depth_boundaries = [0] + depths + [depths[-1] * 2]  # [0, 1, 5, 10]
            
            # Each layer gets one resistivity value
        ...
        ax1.set_xlim([0.1, 1000])  # Reasonable resistivity range
        ...
        # Try simpler inversion without transforms
        print("\nTrying simpler inversion approach...")
        try:
            inv_simple = pg.Inversion(verbose=True)
            inv_simple.setForwardOperator(fop)
            inv_simple.setLambda(0.1)
            inv_simple.setMaxIter(10)
            
            result = inv_simple.run(d_obs, abs_err, startModel=start_model)
            print("Simple inversion successful!")
            print("Result:", result)
            
            # Convert and plot simple results
            result_array = np.array(result)
            result_array = np.clip(result_array, 1e-6, 100)
            resistivity_simple = 1.0 / result_array
            print("Simple inversion resistivity:", resistivity_simple)
            
        except Exception as e2:
            print(f"Simple approach also failed: {e2}")
