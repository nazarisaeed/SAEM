...
class fopEmpyMod(pg.Modelling):
    def __init__(self, depths):
        ...
        # Set parameter bounds to prevent extremely small values
        self.setParameterBounds(lowerBound=1e-3, upperBound=10.0)  # 1mS/m to 10S/m

    def response(self, model):
        # Convert conductivity [S/m] to resistivity [Ohm·m]
        resistivity = 1.0 / np.maximum(np.array(model), 1e-6)  # Prevent division by zero
...
if __name__ == '__main__':
    ...
    # 4. Setup inversion with better parameters
    ...
    try:
        ...
        conductivity_array = np.maximum(conductivity_array, 1e-6)  # Prevent zeros
